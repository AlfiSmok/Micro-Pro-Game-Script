...
void loop() {
  ...
  // Włączenie kierunku (tap przy ruchu manetką)
  if (edgeLewy) {
    tap(BTN_LEWY, "LEWY");
  ...
  if (neutralStable && aktualnyKierunek != NEUTRAL) {
    Serial.println("Powrót do NEUTRAL (bez tap OFF)");
  ...
  if (nowDlugieState != lastDlugieState) {
    ...
    if (nowDlugieState)
      ...
      Serial.println("Światła długie: TAP przy puszczeniu");
  }
